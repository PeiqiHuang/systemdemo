...
$("#roleRightView").treegrid({
    url: "${base}/admin/auth/userRightData.do?suId=${sysUser.suId}",
...
function updateUserDefinedRight(){
	$.messager.confirm("操作提示", "你确定要调整用户权限吗？", function (flag) {
	    if (flag) {
	        $.myAjax.excuteAjaxMask({
				url:"${base}/admin/auth/updateUserDefinedRight.do",
...
<tr><td  class="Search_BtnRow" style="text-align: left">用户ID:<span style="color: #FF8888;padding-right: 12px;">${sysUser.suId }</span>登录名:<span style="color: #FF8888;padding-right: 12px;">${sysUser.loginName }</span>真实姓名:<span style="color: #FFA488;">${sysUser.realName }</span></td><td class="Search_BtnRow" ><a class="easyui-linkbutton" iconCls="icon-redo" href="javascript:$('#roleRightView').treegrid('collapseAll')"  style="width: 80px">折叠</a><a class="easyui-linkbutton" iconCls="icon-save" href="javascript:updateUserDefinedRight()"  style="width: 80px">保存</a></td></tr>
</table>
<hr title="点南隐藏搜索框"  class="searchTabSplit">
<form id="sf"><input type="hidden" name="suId" value="${sysUser.suId }"/>
<table id="roleRightView" title="系统用户-权限视图"  data-options="animate: true,idField:'menuId',treeField: 'menuName'">
	<thead>
		<tr>
